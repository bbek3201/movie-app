...
export const TopRated = ({ skip, setSkip, setTotal, total }: Props) => {
  ...
  return (
    <div
      ...
      // ShowAll үед хоосон зай дээр дарахад хаагдана
    ...
    >
      ...
      {!showAll ? (
        /* 2 ЭГНЭЭГЭЭР ХЭВТЭЭ ГҮЙДЭГ ХЭСЭГ */
        ...
        /* SEE MORE ДАРСАН ҮЕД ХАРАГДАХ GRID */
        <div
          className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-x-6 gap-y-10 animate-in fade-in duration-500"
          onClick={(e) => e.stopPropagation()} // Кино хоорондын зай дээр дарахад хаагдахгүй байх
        >
          {movies.map((movie) => (
            <Link
              key={movie.id}
              href={`/movie/${movie.id}`}
              onClick={(e) => e.stopPropagation()} // Кино руу орох үед контейнерийн onClick-ийг зогсоох
      ...
      {showAll && (
        <div
          ...
          onClick={(e) => e.stopPropagation()} // Pagination дээр дарахад хаагдахгүй
        >
          <button
            onClick={() => goToPage(currentPage - 1)}
            disabled={currentPage === 1}
            className="px-3 py-2 text-sm font-bold text-black dark:text-white hover:opacity-70 disabled:opacity-20 disabled:cursor-not-allowed"
          >
            ‹ Prev
          </button>
          ...
          <div className="flex items-center gap-2">
            {currentPage > 3 && (
              <>
                <button
                  onClick={() => goToPage(1)}
                  className="h-10 w-10 text-gray-400 hover:text-black dark:hover:text-white font-bold"
                >
                  1
                </button>
